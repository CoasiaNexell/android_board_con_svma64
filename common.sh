...
function build_dist()
{
    ...
    local product=PRODUCT-aosp_${2}-${3}
    if [ "${QUICKBOOT}" == "true" ]; then
        cp -f ${TOP}/device/nexell/con_svma64/ota_from_target_files_svm.py ${TOP}/build/tools/releasetools/ota_from_target_files.py
        cp -f ${TOP}/device/nexell/con_svma64/releasetools_svm.py ${TOP}/device/nexell/con_svma64/releasetools.py
        make ${product} TARGET_SOC=${1} QUICKBOOT=1 QPART=1 dist -j8
    else
        cp -f ${TOP}/device/nexell/con_svma64/ota_from_target_files_normal.py ${TOP}/build/tools/releasetools/ota_from_target_files.py
        cp -f ${TOP}/device/nexell/con_svma64/releasetools_normal.py ${TOP}/device/nexell/con_svma64/releasetools.py
...
function make_uboot_bootcmd_svm()
{
    ...
    local bootcmd="mmc read ${load_addr} ${partition_kernel_block_num_hex} ${kernel_block_size};\
		...
		if test !-z \$\{change_devicetree\}; then run change_devicetree; fi;\
        booti ${kernel_start_hex} - ${dtb_dest_addr}"
    echo -n ${bootcmd}
}
##
# args
